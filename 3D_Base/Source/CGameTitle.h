...
public:
	...
	std::shared_ptr<CBody>				m_pPlayerBody;
	std::shared_ptr<CCannon>			m_pPlayerCannon;

	//プレイヤーのメッシュ.
	std::shared_ptr<CStaticMesh>		m_pCannon;
	std::shared_ptr<CStaticMesh>		m_pBody;

	//地面クラス.
	std::shared_ptr<CGround>			m_Ground;
	//地面のメッシュ.
	std::shared_ptr<CStaticMesh>		m_GroundMesh;
	float y = 0.f;
