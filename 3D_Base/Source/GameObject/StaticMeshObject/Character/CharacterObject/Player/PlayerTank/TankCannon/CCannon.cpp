...
bool CCannon::RaycastToPosition(const D3DXVECTOR3& targetPos, float targetRadius, float& outDistance) const
{
    D3DXVECTOR3 muzzle = GetMuzzlePosition();
    D3DXVECTOR3 forward = GetForward();
    ...
    D3DXVECTOR3 lateral = targetPos - projected;
    lateral.y = 0.0f;
    float lateralDist = D3DXVec3Length(&lateral);

    // ターゲットの半径内ならヒット
    if (lateralDist <= targetRadius)
    {
